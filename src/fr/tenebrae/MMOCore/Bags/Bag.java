...
public class Bag {
	...
	public Bag(String serializedBag) {
		try {
			...
			Main.log.info("Size is "+infos[0]);
			Main.log.info("Content is "+infos[1]);
	...
	public void clear() {
		...
		if (size%9 != 0) {
			for (int k = 0; k < size%9; k++) {
				content.setItem(content.getSize()-k, getLocker());
			}
		}
	}
